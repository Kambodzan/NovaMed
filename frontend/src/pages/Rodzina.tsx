  ...
  return (
    <div className="space-y-6">
      <div className="fade-up">
        <PageHeader
          overline="Konta rodzinne"
          title="Rodzina"
          sub="Profile podopiecznych — umawiaj wizyty i przeglądaj ich dokumentację w ich imieniu"
        />
      </div>

      <Tile className="p-5" delay={60}>
        <TileHeader title={<span className="inline-flex items-center gap-1.5"><Users size={13} /> Podopieczni</span>} />
        {dependents.length === 0 ? (
          <EmptyState
            icon={<Users size={28} strokeWidth={1.5} />}
            title="Brak podopiecznych"
            hint="Dodaj profil dziecka lub osoby pod opieką formularzem poniżej."
          />
        ) : (
            ...
            {dependents.map(d => (
              <li key={d.patient_id} className="flex flex-wrap items-center gap-3 rounded-2xl bg-gray-50 px-4 py-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs font-medium text-gray-500">PESEL {d.pesel} · ur. {formatDatePL(d.birth_date)}</p>
                </div>
                {activeId === d.patient_id ? (
                  <Button size="sm" variant="secondary" onClick={() => setActiveId(null)}>Wróć na mój profil</Button>
                ) : (
                  <Button size="sm" onClick={() => setActiveId(d.patient_id)}>Przełącz na ten profil</Button>
                )}
              </li>
            ))}
          </ul>
      ...
      <Tile className="p-5" delay={120}>
        <TileHeader title="Dodaj podopiecznego" />
        <form
        ...
        >
          ...
          <div className="flex items-end">
            <Button disabled={add.isPending} type="submit"><Plus size={15} /> Dodaj</Button>
          </div>
        </form>
